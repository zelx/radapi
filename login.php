...
if(isset($_GET['res'])){
		switch ($_GET['res']){
			case 'notyet': 			display_notyet();
									break;
			case 'failed':			display_failed();
									break;
			case 'success':			display_success();
									break;
			case 'logoff':			display_logoff();
									break;
			case 'timeout':			display_logoff();
									break;
			case 'already': 		display_already();
									break;
			case 'success_popup': 	display_success_popup();
									break;
			default:	echo "Unknow";
		}

}else if(!isset($_GET['chal'])){

  $result = 0;
  $headline = 'HotSpot Login Failed';
  $bodytext = 'Login must be performed through Wifi daemon!';
...
//  	$newpwd = pack("a32", $_GET['password']);
	...
	$response = md5("\0" . substr(md5($_GET['password']),0,15) . $newchal);
  ...
  <body>
    ...
    <div class="container">
     ...
     <form class="form-signin">
	   ...
	   <p class="text-error" align=center></p>
