		  ...
		  if($data2->gexpire == "0000-00-00" || $data2->gexpire >= date("Y-m-d")) {
			  ...
			  switch($data0->encryption) {
				...
				case 'clear' : $response = md5("\0" . $_GET['Password'] . $newchal);
							  ...
							  break;
				case 'crypt' : $response = md5("\0" . crypt($_GET['Password'],"BL") . $newchal);
							  $newpwd = pack("a32", crypt($_GET['Password'],"BL"));
							  break;
				case 'md5ums' : $response = md5("\0" . substr(md5("O]O" . $_GET['Password'] . "O[O"),0,15)  . $newchal);
							  $newpwd = pack("a32", substr(md5("O]O" . $_GET['Password']. "O[O"),0,15));
							  break;
			  }
		  } else {
			  $newpwd = "groupexpire";
...
if ($_GET['res'] == already) {
  ...
  $bodytext = '<BR><BR><BR><input type="button" value="ออกจากระบบ" class="button" onclick="window.location=\'http://' . $_GET['uamip'] . ':' . $_GET['uamport'] . '/logoff\'">';
		...
		<script language=\"JavaScript\">
			...
			function popUp(URL) {
			  if (self.name != \"chillispot_popup\") {
				chillispot_popup = window.open(URL, 'chillispot_popup', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=600,height=400');
			  }
			}
...
function print_body(){
  ...
  <body onLoad=\"javascript:doOnLoad($result, '".$loginpath_popup."?res=popup2&uamip=$uamip&uamport=$uamport&userurl=$userurl&redirurl=$redirurl&timeleft=$timeleft','$userurldecode', '$redirurldecode', '$timeleft')\" onBlur = \"javascript:doOnBlur($result)\">
...
function print_login_form(){
  ...
  print '<FORM name="form1" METHOD="get" action="' . $loginpath . '?">
          ...
          <input type="HIDDEN" NAME="login" value="login">
			...
			<input type="button" name="button2" value="เปลี่ยนรหัสผ่าน" onClick=window.location="admin/changepass.php" class="button">
			</p>
